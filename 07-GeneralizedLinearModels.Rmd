...
# 1. Get MLEs from the fit
mu_mle <- fit$estimates["mu"]
k_mle <- fit$estimates["k"]

# 2. Get the full range of observed counts
max_y <- max(corbet$ofreq)
y_range <- 1:max_y

# 3. Calculate Empirical Probabilities
N_total <- sum(corbet$species)
# Create a full vector, including 0s for unobserved counts (e.g., y=23)
emp_probs_full <- rep(0, max_y)
emp_probs_full[corbet$ofreq] <- corbet$species / N_total

# 4. Calculate Fitted Probabilities for the ZTNB
# P(Y=0) for the underlying untruncated NB
p0 <- dnbinom(0, size = k_mle, mu = mu_mle)

# Probabilities for y = 1, 2, ..., max_y
# P(Y=y | Y>0) = P(Y=y) / (1 - P(0))
fitted_probs_full <- dnbinom(y_range, size = k_mle, mu = mu_mle) / (1 - p0)

# 5. Create the Plot
# Set a plot limit (x-axis) to see the main distribution.
# The 'corbet' data has long tails that can skew the visual.
plot_limit <- 30 # Adjust as needed
y_range_plot <- 1:plot_limit

# Open a new graphics window (optional, but good practice)
if(dev.cur() == 1) dev.new() # Avoid overwriting other plots

# Plot the empirical probabilities as a histogram
plot(y_range_plot, emp_probs_full[y_range_plot], 
     type = "h", # 'h' for histogram-like vertical lines
     lwd = 3,
     col = "blue",
     xlab = "Number of Individuals (y)",
     ylab = "Probability / Proportion",
     main = "Corbet Data: Empirical vs. Fitted ZTNB Model",
     ylim = c(0, max(emp_probs_full, fitted_probs_full)),
     xlim = c(0, plot_limit + 1)
)

# Overlay the fitted probabilities as points
points(y_range_plot, fitted_probs_full[y_range_plot], 
       col = "red",
...
# Add a legend
legend("topright", 
       legend = c("Empirical Data", "Fitted ZTNB Probabilities"),
       col = c("blue", "red"),
       pch = c(NA, 19),      # No point for the line
       lwd = c(3, NA),       # Line width for the 'h' plot
