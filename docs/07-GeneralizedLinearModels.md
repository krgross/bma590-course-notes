...
We would like fit a zero-truncated negative binomial distribution to these data, and compare the fitted zero-truncated distribution to the data.

As of Nov. 2025, the `VGAM::vglm` routine throws a [known error](https://stackoverflow.com/questions/79702156/error-could-not-find-function-dposnegbin2-when-using-posnegbinomial-in-vgl) when fitting these data.  It shouldn't be hard to write code to maximize the likelihood directly.  Here's Gemini code for producing the fit:


``` r
# R code to fit a Zero-Truncated Negative Binomial (ZTNB) distribution
# using only base R's 'optim' function.

#' Calculate the Negative Log-Likelihood for a ZTNB
#'
#' This function calculates the total negative log-likelihood for a dataset
#' given the parameters (mu, k) of the underlying (untruncated)
#' negative binomial distribution.
#'
#' @param params A numeric vector c(mu, k)
#' @param y A numeric vector of observed counts (y = 1, 2, 3, ...)
#' @param freq A numeric vector of frequencies for each count
#' @return The negative log-likelihood (a single numeric value)

negLogLik_ZTNB <- function(params, y, freq) {
    
    mu <- params[1]
    k <- params[2]

    # Parameters must be positive.
    # 'optim' with L-BFGS-B handles this, but as a safeguard:
    if (mu <= 0 || k <= 0) {
        return(1e9) # Return a large value
    }

    # 1. Calculate the probability of P(Y=0) from the *untruncated* NB
    # dnbinom(x, size = k, mu = mu) is the base R function
    p0 <- dnbinom(0, size = k, mu = mu)

    # 2. Calculate the probability of P(Y > 0)
    # We use log1p(-p0) which is log(1 - p0) for numerical stability
    log_p_not_zero <- log1p(-p0)

    # Stability check: if p0 is 1 (or very close), log(1-p0) is -Inf.
    # This happens with bad parameters.
    if (is.infinite(log_p_not_zero)) {
        return(1e9)
    }

    # 3. Calculate the log-likelihood for each observed y (y=1, 2, 3...)
    # The log-likelihood for a *single* ZTNB observation is:
    # log(P(Y=y | Y>0)) = log( P(Y=y) / (1 - P(Y=0)) )
    #                  = log(P(Y=y)) - log(1 - P(Y=0))
    #
    # We use log = TRUE for numerical stability.
    log_probs_vec <- dnbinom(y, size = k, mu = mu, log = TRUE)

    # 4. Calculate the total log-likelihood for the *entire* dataset
    # This is the sum of weighted log-likelihoods.
    # LL = SUM( freq[i] * (log(P(y[i])) - log(1 - P(0))) )
    #
    # By algebra, this is:
    # LL = SUM( freq[i] * log(P(y[i])) ) - SUM( freq[i] * log(1 - P(0)) )
    # LL = SUM( freq[i] * log(P(y[i])) ) - N * log(1 - P(0))
    # where N is the total number of species.
    
    N <- sum(freq)
    sum_weighted_log_probs <- sum(freq * log_probs_vec)
    
    total_log_lik <- sum_weighted_log_probs - N * log_p_not_zero
    
    # 6. Return the NEGATIVE log-likelihood for minimization
    # optim() minimizes, so we return -LL
    return(-total_log_lik)
}

#' Fit ZTNB using Base R's optim
#'
#' Main wrapper function to find the MLEs for mu and k.
#'
#' @param y_counts The vector of counts (e.g., corbet$ofreq)
#' @param y_freqs The vector of frequencies (e.g., corbet$species)
#' @return A list containing the optimization results.
fit_ztnb_base <- function(y_counts, y_freqs) {
    
    # 1. Get starting values for mu and k
    # We use the method of moments on the *observed* (truncated) data
    # as a rough approximation.
    obs_mean <- weighted.mean(y_counts, y_freqs)
    
    # Calculate weighted variance
    obs_var <- sum(y_freqs * (y_counts - obs_mean)^2) / (sum(y_freqs) - 1)

    # For a standard NB (not ZTNB), k = mu^2 / (Var - mu)
    # This is just a starting guess.
    mu_start <- obs_mean
    k_start <- if (obs_var > mu_start) {
        mu_start^2 / (obs_var - mu_start)
    } else {
        1.0 # Default to 1 if variance is not > mean
    }
    
    start_params <- c(mu = mu_start, k = k_start)
    
    # 2. Run the optimization
    # We use "L-BFGS-B" method, which allows box constraints (lower bounds)
    # This is crucial as mu and k *must* be positive.
    
    optim_result <- optim(
        par = start_params,
        fn = negLogLik_ZTNB,
        # Pass additional arguments to negLogLik_ZTNB
        y = y_counts,
        freq = y_freqs,
        # Use L-BFGS-B for bounded optimization
        method = "L-BFGS-B",
        # Set lower bounds for mu and k
        lower = c(1e-6, 1e-6)
    )
    
    # 3. Format and return the results
    if (optim_result$convergence != 0) {
        warning("Optimization may not have converged. Code: ", 
                optim_result$convergence)
    }
    
    estimates <- optim_result$par
    names(estimates) <- c("mu", "k")
    
    return(list(
        estimates = estimates,
        log_likelihood = -optim_result$value, # Return the actual LL
        convergence = optim_result
    ))
}
# 3. Run the fitting function
#    ofreq = the count (y)
#    species = the frequency (freq)
fit <- fit_ztnb_base(y_counts = corbet$ofreq, y_freqs = corbet$species)
...
Here's a plot of the fit compared to the empirical frequency distribution:
...
The `pscl` library contains the `hurdle` function for fitting ZA modes.  However, in ZA models, the two components are entirely separate from one another, so one doesn't really need additional specialized software.  ZA models could be fit just as readily by bolting together a logistic regression for the hurdle component with a model for zero-truncated responses in the count component.  This is not true for ZI models, though.  For ZI models, the estimated proportion of structural zeros affects the estimate of the intensity for the non-structural-zero counts, so the two components cannot be fit separately from one another.
